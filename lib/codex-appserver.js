        ...
        while ((newlineIndex = buffer.indexOf('\n')) >= 0) {
          ...
          if (message.id === initializeId) {
            ...
            const resumeParams = {
              ...
              approvalPolicy: approvalPolicyForMode(mode),
              approvalsReviewer: 'user',
            };
            if (model) resumeParams.model = model;
            if (reasoningEffort) resumeParams.config = { model_reasoning_effort: reasoningEffort };
            resumeId = writeRequest('thread/resume', resumeParams);
          } else if (message.id === resumeId) {
            if (message.error) return finish(new Error(message.error.message || 'Codex 会话恢复失败'));
